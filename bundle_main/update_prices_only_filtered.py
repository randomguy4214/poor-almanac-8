#!/usr/bin/python
print('update_prices_filtered - initiating.')
...
last_ticker = pd.read_csv(os.path.join(cwd,input_folder,temp_folder,"prices_last_ticker_filtered.csv"),index_col=0)
last_ticker_n = last_ticker.values[0]
last_ticker_nn = last_ticker_n[0]
print("last ticker in prices_filtered was number", last_ticker_nn)
...
for t in tickers.split(' '):
    try:
        ...
        if n > last_ticker_nn:
            ...
            last_ticker.to_csv(os.path.join(cwd, input_folder, temp_folder, "prices_last_ticker_filtered.csv"))
    except:
        pass
...
print('update_prices_filtered - done')
