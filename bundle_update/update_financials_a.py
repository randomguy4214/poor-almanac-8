...
for t in tickers.split(' '):
    try:
        ...
        if n >= last_ticker_n:
            ...
            df_merged = pd.merge(df_inc, df_bs, how='left', left_on=['calendarYear'], right_on=['calendarYear'],
                                 suffixes=('', '_drop'))
            df_merged.drop([col for col in df_merged.columns if 'drop' in col], axis=1, inplace=True)
            df_to_merge = df_merged
            df_merged = pd.merge(df_to_merge, df_cf, how='left', left_on=['calendarYear'], right_on=['calendarYear'],
                                 suffixes=('', '_drop'))
            df_merged.drop([col for col in df_merged.columns if 'drop' in col], axis=1, inplace=True)
            df = df_merged
            df['symbol'] = t
